...
| `proxy`                     | undefined   | `proxy` is a batch URL to post the events. Enable `useSegmentEndpoints` if proxy domain is provided and you want to append the Segment endpoints automatically. If you want to completely customize the proxy by providing a custom URL, disable `useSegmentEndpoints`. Default value is `false`.                                                                                                                                                                                                                                       |
| `errorHandler`              | undefined   | Create custom actions when errors happen, see [Handling errors](#handling-errors).                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `cdnProxy`                  | undefined   | Sets an alternative CDN host for settings retrieval. Enable `useSegmentEndpoints` if `cdnProxy` domain is provided and you want to append the Segment endpoints automatically. <br><br>⚠️ **IMPORTANT NOTE:** _Prior to version 2.20.4, any value provided for `cdnProxy` was automatically appended with `/write-key/settings`. **Starting from v2.20.4**, this behavior has **CHANGED**—the SDK will now behave based on the `useSegmentEndpoints` flag._ Please update your configuration accordingly to avoid unexpected issues. ⚠️ |
| `useSegmentEndpoints`        | false       | Set to `true` to automatically append the Segment endpoints when using `proxy` or `cdnProxy` to send or fetch settings. This will enable automatic routing to the appropriate endpoints.                                                                                                                                                                                                                                                                                                                                              |
