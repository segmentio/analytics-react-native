...
- `ci-e2e-nightly.yml`: scheduled 06:00 UTC (and manual dispatch). Uses the flox install action and runs full matrix: iOS min (runtime 15.0) and max (26.1), Android min (API 21) and max (API 33). Uses flox services for bootstrapping devices. Targets come from `env/common/.flox/env/manifest.{toml,lock}` via `scripts/targets.sh` so min/max stay in sync. Xcode is installed via `setup-xcode@v1` (26.1.1); required runtimes download if missing, and local runs can use whatever Xcode is available.
