...
describe('AmplitudeSessionPlugin', () => {
  ...
  const setupPluginWithClient = async () => {
    ...
    plugin.update({ integrations: { 'Actions Amplitude': {} } } as SegmentAPISettings, UpdateType.initial);
  ...
  describe('startNewSession scenarios', () => {
    ...
    it('should NOT start a new session when session is still active', async () => {
      ...
      expect(plugin.analytics?.track).not.toHaveBeenCalledWith('session_start', expect.any(Object));
    });
  });
  ...
  describe('bug detection: multiple startNewSession calls', () => {
    ...
    it('BUG: should detect multiple session starts for rapid events (currently masked by 1000ms guard)', async () => {
      ...
      console.warn('🐛 BUG MASKED: Multiple session start attempts should never occur');
    });
    ...
    it('BUG: should detect race conditions in parallel event execution', async () => {
      ...
      if (sessionStartCalls.length > 1) {
        console.error(`🐛 BUG DETECTED: ${sessionStartCalls.length} session_start calls for parallel events`);
    ...
    it('BUG: should detect inconsistent session state', async () => {
      ...
      if (sessionStartCalls.length > 1 || sessionEndCalls.length > 1) {
        console.error(`🐛 BUG DETECTED: Inconsistent state caused multiple session events - starts: ${sessionStartCalls.length}, ends: ${sessionEndCalls.length}`);
      }
    ...
    it('BUG: should detect async race conditions in startNewSessionIfNecessary', async () => {
      ...
      const startNewSessionIfNecessarySpy = jest.spyOn(plugin as any, 'startNewSessionIfNecessary');
      const endSessionSpy = jest.spyOn(plugin as any, 'endSession');
      ...
      const startNewSessionIfNecessaryCalls = startNewSessionIfNecessarySpy.mock.calls.length;
      const endSessionCalls = endSessionSpy.mock.calls.length;
      const startNewSessionCalls = startNewSessionSpy.mock.calls.length;
      ...
      console.log(`📊 Session operations: startNewSessionIfNecessary: ${startNewSessionIfNecessaryCalls}, endSession: ${endSessionCalls}, startNewSession: ${startNewSessionCalls}`);
      
      if (startNewSessionCalls > 1) {
        console.error(`🐛 CRITICAL BUG DETECTED: ${startNewSessionCalls} startNewSession calls from concurrent execute operations`);
        // This should fail if there are race conditions
        expect(startNewSessionCalls).toBe(1);
      }

      if (endSessionCalls > 1) {
        console.error(`🐛 BUG DETECTED: ${endSessionCalls} endSession calls from concurrent operations`);
    ...
    it('BUG: should detect overlapping session end/start operations', async () => {
      ...
      if (startNewSessionCalls > 1) {
        console.error(`🐛 CRITICAL BUG DETECTED: ${startNewSessionCalls} startNewSession calls from concurrent operations`);
        expect(startNewSessionCalls).toBe(1);
      }

      // Verify the track calls
      const trackMock = plugin.analytics?.track as jest.Mock;
      const sessionEndCalls = trackMock.mock.calls.filter((call: any) => call[0] === 'session_end');
      const sessionStartCalls = trackMock.mock.calls.filter((call: any) => call[0] === 'session_start');

      if (sessionEndCalls.length > 1 || sessionStartCalls.length > 1) {
        console.error(`🐛 BUG DETECTED: Multiple session events - ends: ${sessionEndCalls.length}, starts: ${sessionStartCalls.length}`);
      }
  ...
  expect(plugin.sessionId).toBe(oldSessionId);
  expect(plugin.analytics?.track).not.toHaveBeenCalledWith('session_start', expect.any(Object));
  expect(plugin.analytics?.track).not.toHaveBeenCalledWith('session_end', expect.any(Object));
  ...
  describe('session data persistence', () => {
    ...
    it('should clear session data on reset', async () => {
      ...
      expect(mockAsyncStorage.removeItem).toHaveBeenCalledWith('previous_session_id');
      expect(mockAsyncStorage.removeItem).toHaveBeenCalledWith('last_event_time');
    });
  });
